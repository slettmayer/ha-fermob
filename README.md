...
| `update.<lamp>_firmware` | Update (`firmware`) | — | The lamp's firmware version, and whether the manufacturer has published a newer one. **No install button** — see [Firmware updates](#firmware-updates). Absent if you switch the check off |
...
Switching it off removes the `update` entity along with the request. Your lamp's
firmware version still shows on the device page: that comes from the lamp itself.
