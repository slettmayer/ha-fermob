...
| `fermob.unpair` | Broadcasts `UNREGISTER`, deletes the stored keys and removes the config entry. The lamp flashes 3× and resets its crypto state, so the Fermob app can claim it again |
