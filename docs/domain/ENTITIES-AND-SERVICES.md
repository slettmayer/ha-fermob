...
| **Check for firmware updates** | On | Whether the firmware entity is enabled and asks the vendor server whether a newer build is published. **The integration's only non-local traffic**, which is the whole reason it is switchable; off *disables* the entity rather than deleting it; see [FIRMWARE-UPDATE.md](FIRMWARE-UPDATE.md) |
